...
def stabilize_drone(self, speed):
    ...
    def PID_hover(self, target_pitch, target_roll, target_yaw, target_altitude):
        ...
        yaw_error = target_yaw - drone.get_yaw()
        ...
        yaw_control = 1 * yaw_error      # P term only
        altitude_control = 1 * altitude_error  # P term only

        # Adjust motor speeds based on control outputs
        a = altitude_control + pitch_control + roll_control + yaw_control
        b = altitude_control + pitch_control - roll_control - yaw_control
        c = altitude_control - pitch_control + roll_control - yaw_control
        d = altitude_control - pitch_control - roll_control + yaw_control
